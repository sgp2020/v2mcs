...
$(function() {
  ...
  function searchAtomicTransferLogList(commandId) {
      ...
      var success = function(retObj) {
    	  atomicTransferLogTable.clear();
    	  atomicTransferLogTable.addDataList(retObj.body);
      };
  ...
  //atomic_transfer_log  getAtomicLogDataテーブル
  var atomicTransferLogTable = new McsTable($('#atomicTransferLog-table-target'));
  atomicTransferLogTable.setNotRowSelect(true);

  // 状態テーブルヘッダ(状態テーブル)
  var atomicTransferLogDataHeader = [{
	    name: 'rum',
  ...
  // ヘッダ設定(状態テーブル)
  atomicTransferLogTable.setHeader(atomicTransferLogDataHeader);
  atomicTransferLogTable.setBodyHeight($('.mcs-content.mcs-with-subheader.mcs-with-subtitle').outerHeight() - 40);
  /**
   ******************************************************************************
   * @brief   抽出して画面へ表示する
